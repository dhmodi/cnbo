...
import com.deloitte.kafka.util.KafkaETLInputFormat;
import com.deloitte.kafka.util.KafkaETLJob;
...
public class SimpleKafkaETLJob {
	...
	//Sanjana
//	protected JobConf createJobConf_offset() throws Exception {
//		JobConf jobConf_offset = KafkaETLJob.createJobConf("SimpleKafakETL", _topic, _props, getClass());		
		
//		jobConf_offset.setOutputFormat(TextOutputFormat.class);
//		TextOutputFormat.setCompressOutput(jobConf_offset, false);
//		Path output = new Path("/tmp/kafka/offset");
//		FileSystem fs = output.getFileSystem(jobConf_offset);
//		if (fs.exists(output)) fs.delete(output);
//		TextOutputFormat.setOutputPath(jobConf_offset, output);
		
//		return jobConf_offset;
//	}
...
//        JobConf conf_offset = createJobConf_offset();
//       RunningJob runningJob_offset = new JobClient(conf_offset).submitJob(conf_offset);
//       String id_offset = runningJob_offset.getJobID();
//        runningJob_offset.waitForCompletion();
        
//        if (!runningJob_offset.isSuccessful()) 
//            throw new Exception("Hadoop offset job failed! Please check status on http://"
//                                         + conf.get("mapred.job.tracker") + "/jobdetails.jsp?jobid=" + id_offset);
     //Sanjana   
    }
